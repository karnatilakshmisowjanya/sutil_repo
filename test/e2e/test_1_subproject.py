...
def test_sdutil_patch_subproject(capsys, pargs):
    import json
    import sys
    path = pargs.sdpath
    tenant,subproject = path.split("/")[2],path.split("/")[3]
    ...
    errors = verify_conditions( sdutil_patch_subproject = str(sdutil_patch_status) + ';' + sdutil_patch_output + 'Command: ' + sys.argv,
