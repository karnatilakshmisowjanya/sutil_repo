...
@StorageFactory.register(provider="gc")
class GcStorageService(StorageService):
    __STORAGE_CLASSES = ['REGIONAL', 'MULTI_REGIONAL', 'NEARLINE', 'COLDLINE']
    __STORAGE_LOCATION_MR = ['ASIA', 'EU', 'US']
    __GOOGLE_API_BASE_URL = 'https://www.googleapis.com/'
    __STORAGE_EP = 'storage/v1'
    __STORAGE_UPlOAD_EP = 'upload/storage/v1'

    def __init__(self, auth):
        super(GcStorageService, self).__init__(auth=auth)
    ...
    def object_upload(self, bucket, objname, data, tenant, subproject):
        relative_url = f"{self.__STORAGE_UPlOAD_EP}/b/{bucket}/o?uploadType=media&name={quote(objname, safe='')}"
        url = urljoin(self.__GOOGLE_API_BASE_URL, relative_url)
    ...
    def upload_resumable_start(self, bucket, objname, dataset, totsize):
        relative_url = f"{self.__STORAGE_UPlOAD_EP}/b/{bucket}/o?uploadType=resumable&name={quote(objname, safe='')}"
        url = urljoin(self.__GOOGLE_API_BASE_URL, relative_url)
    ...
    def object_download(self, bucket, obj, tenant,
        ...
        base_url = f"https://{bucket}.storage.googleapis.com/"
        relative_url = f"{quote(obj, safe='')}?alt=media"
    ...
    def object_attribute(self, bucket, objname, attribute, tenant, subproject):
        relative_url = f"{self.__STORAGE_EP}/b/{bucket}/o/{quote(objname, safe='')}?fields={attribute}"
        url = urljoin(self.__GOOGLE_API_BASE_URL, relative_url)
    ...
    def download(self, localfilename, dataset):
        ...
        with open(localfilename, 'wb') as lfile:
            ...
            for obj in range(0, nobjects):
                cursize_incr, crc32c_local = self.downloadObject(lfile, bucket, object_path + '/' +
                                                                 str(obj), dataset, cursize)
                crc32c_remote = self.object_attribute(bucket, object_path + '/' +
    ...
    def get_storage_classes():
        return GcStorageService.__STORAGE_CLASSES

    @staticmethod
    def get_storage_multi_regions():
        return GcStorageService.__STORAGE_LOCATION_MR
